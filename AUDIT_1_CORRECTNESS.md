...
3. **Invoice Edit button UI polish** *(opened 2026-05-12, commit `b108f0e`)*. The Edit button added to each row on `/invoices` sits visually cramped between the status `Select` and the Delete button. Per Njuguna (2026-05-12 ~02:08 EAT): "I want the edit button to be more clearer and look neater." Resolution: design pass on the row action cluster. Options to consider: (a) consolidate Edit + Delete into a kebab menu; (b) explicit icon-only buttons with tooltips; (c) better spacing and visual hierarchy with the existing flat buttons. Not blocking any functionality; the button works end-to-end (verified by INV-32 edit smoke test 2026-05-11 23:10:29 UTC).
