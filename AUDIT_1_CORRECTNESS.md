...
0.1. **RESOLVED (2026-05-12 ~02:15 EAT) — Lagged revenue investigation closed.** May 2026 P&L showed zero revenue because Windward's INV-32 (KES 4,133,640 / USD 31,920) was saved with `billing_period = '2026-05'` not `'2026-04'`. Fixed via direct SQL UPDATE during the same session (audit row id `467b1a38-5499-48f9-846d-e5a6e0e07b43` in `audit_logs` captures the change). Going forward the Edit Invoice feature shipped this session (commit `b108f0e`) provides a sanctioned UI path for operators to correct `billing_period` mistakes. Diagnosis artifact: `LAGGED_REVENUE_DIAGNOSIS.md` at repo root (untracked).
...
2. **Edit Invoice feature ships with `user_id = NULL` in `audit_logs`** *(opened 2026-05-12, commit `b108f0e`)*. Invoice edits through `/api/invoices/update` write `audit_logs` rows where `user_id` is NULL because the route uses the service-role admin client, which has no `auth.uid()` value to feed the `audit_invoices` trigger. Mutation context is fully preserved in `old_values` / `new_values` JSONB; only operator identity is missing. Same class of gap as the existing "`fn_audit_log` session-GUC `user_id` fallback (Item 2)" ticket. Acceptable resolution path: route sets `app.user_id` GUC before the UPDATE and the trigger reads from that GUC with `auth.uid()` as fallback. Defer for joint resolution with the Item 2 audit-trigger ticket. Verified affecting INV-32 edit on 2026-05-11 23:10:29 UTC (audit row `92b17097-a59a-4d09-9eb2-39dc83b137d8`).
3. **RESOLVED (2026-05-12 ~02:50 EAT) — Invoice Edit button UI polish.** Shipped via commit `b18bcd8`. Row action cluster on `/invoices` consolidated: Status `Select` moved into col 6 (replacing the static pill for `canManage` roles); col 7 became a kebab `DropdownMenu` containing Edit + Delete (Delete with destructive tint). Matches the `budgets-list-row.tsx:237-303` pattern. Visual confirmation by Njuguna: "LOOKS CLEAN AND PERFECT." Design artifacts: `UI_INVOICE_ROW_ACTIONS_DIAGNOSIS.md` and `UI_INVOICE_ROW_ACTIONS_DESIGN.md` at repo root (untracked).

3.1. **Revenue page `InvoiceRow` consumer uses legacy `actions` slot — refactor to `canManage`/`onEdit`/`onDelete` pattern.** *(opened 2026-05-12 by commit `b18bcd8`)*. The Phase 3 build for the row-actions polish discovered a second consumer of `<InvoiceRow>` at `src/app/(dashboard)/revenue/page.tsx:745` that still passes the legacy `actions={...}` slot. To avoid expanding scope at session end, `actions` was retained on `InvoiceRowProps` as an additive prop with explicit precedence (legacy `actions` wins over the new kebab when both are provided). `statusValue` was also made optional to keep the legacy caller building. Acceptance: `revenue/page.tsx:745` migrates to the new prop shape (`canManage`/`statusValue`/`onStatusChange`/`onEdit`/`onDelete`); the legacy `actions?: React.ReactNode` prop is removed from `InvoiceRowProps`; `statusValue` becomes required again. Also worth considering at that time: a consistency sweep across other list pages (withdrawals especially, which uses the same `h-7 px-2 text-[11px]` ghost pattern) to adopt the same kebab + status-select shape — ticket A2 from the row-actions scope lock.
4. **Removal of Path A/B application-layer UPDATEs** *(opened by Item 1)*. Why deferred: Item 1's trigger writes the same values; client UPDATEs are now harmless no-ops, but removal is scheduled "after ≥2 weeks of clean soak" to preserve a safety net for an emergency trigger rollback. Acceptance: `revenue/page.tsx` and `payment-form-dialog.tsx` no longer issue an `invoices` UPDATE after a payment is recorded; the drift query in `AUDIT1-ITEM1-DESIGN.md §4 Q1` returns 0 rows.
5. **`payment_status` enum collapse into `status`** *(opened by Item 1)*. Why deferred: out of scope for Item 1's race-window close; touches consumer code across multiple files. Acceptance: `invoices.payment_status` column is dropped; all readers source the value from `invoices.status` (the `invoice_status` enum); the trigger writes only `status` thereafter.
6. **`fn_audit_log` `search_path` hardening retrofit** *(opened by Item 1)*. Why deferred: today's two new RPCs (`fn_payments_recompute_invoice`, `fn_budget_items_sync`) set `SET search_path = public, pg_temp`; the legacy `fn_audit_log` (shared across 20+ tables as `SECURITY DEFINER`) does not. Filed as a separate ticket because the retrofit needs verification across all callers. Acceptance: `fn_audit_log` declaration carries `SET search_path = public, pg_temp` and all `audit_*` triggers continue to fire correctly across every audited table.
7. **`fn_audit_log` session-GUC fallback for `user_id`** *(opened by Item 2)*. Why deferred: trigger rows currently carry `user_id = NULL` when the calling client is service-role (`auth.uid()` is NULL under service-role). The proposed `current_setting('app.current_user_id', true)::uuid` fallback restores caller attribution. Filed as a separate ticket because `fn_audit_log` is shared across 20+ tables. Acceptance: trigger rows written under service-role calls carry the authenticated caller's id where the route surfaces it.
8. **`audit_logs` shape split (`old_values` / `new_values` clean separation)** *(opened by Item 2)*. Why deferred: Phase 2 kept the route-compatible shape (`old_values = NULL`, full payload in `new_values` including a `before` key) to avoid changing the read shape for any future log consumer. A cleaner split is contingent on a downstream-consumer audit of `audit_logs`. Acceptance: the consumer audit lands first; then the split moves the snapshot into `old_values` and the diff into `new_values`.
...
10. **Item 1 drift-check verification SQL — operational follow-up** *(opened by Item 1)*. Why deferred: an operational SOP, not a code change. Acceptance: the drift query in `AUDIT1-ITEM1-DESIGN.md §4 Q1` is run against production weekly for ≥2 weeks and returns 0 rows each time.
11. **Audit-4 (PS-1 / PS-2) — locate or author the scope doc** *(surfaced during 2026-05-11 status reconstruction, not strictly opened by Item 1/Item 2)*. Why deferred: migration `00055` references "Audit-4 findings PS-1 and PS-2 from the post-marathon correctness sweep," but no matching scope doc exists in the repo. Acceptance: the canonical Audit-4 scope doc is either found and committed alongside the F-NN catalogue, or authored fresh from PS-1/PS-2's commit history.
