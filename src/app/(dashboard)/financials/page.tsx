...
export default function FinancialsPage() {
  ...
  const d = data;
  const healthColor = d?.health.score_band === 'healthy' ? 'text-success-soft-foreground bg-success-soft border-success/40' : d?.health.score_band === 'watch' ? 'text-warning-soft-foreground bg-warning-soft border-warning/40' : 'text-danger-soft-foreground bg-danger-soft border-danger/40';
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-2">
                {d.hints.map((h, i) => (
                  <div key={i} className={`flex items-start gap-3 rounded-md p-3 ${h.severity === 'critical' ? 'bg-danger-soft border border-danger/40' : h.severity === 'warning' ? 'bg-warning-soft border border-warning/40' : h.severity === 'positive' ? 'bg-success-soft border border-success/40' : 'bg-muted/50 border'}`}>
